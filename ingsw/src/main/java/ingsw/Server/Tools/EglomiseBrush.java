 ...
 * It allows the user to move a Die from a cell to another without respecting color constraints
 *
 *  Usage first run:  T-2 [x1-y1] [x2-y2]   optional: D-N for single player
 */
public class EglomiseBrush extends GenericTool{

    public EglomiseBrush() {
        this(GameType.MULTIPLAYER);
    }

    public EglomiseBrush(GameType gameType) {
        super(gameType);
        this.toolName = "Eglomise Brush";
    ...
    public boolean isUsable(ToolActionParameter tap, GameBoard gameBoard) throws GameException {
        ...
        boolean check1 = playerGrid.isValidNearShade(toMove, coords.get(1).x, coords.get(1).y);
        boolean check2;
